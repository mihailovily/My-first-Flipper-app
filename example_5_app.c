...
static void example_5_app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    if(mode == DRAW_ONLY_PICTURES) canvas_draw_icon(canvas, 0, 29, &I_amperka_ru_logo_128x35px);
    if(mode == DRAW_ONLY_TEXT || mode == DRAW_ALL) {
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str(canvas, 0, 8, "Learn and do!");
        canvas_set_font(canvas, FontSecondary);
        elements_multiline_text_aligned(
            canvas, 0, 15, AlignLeft, AlignTop, "Hello from\nmihailovily");
    }
}
